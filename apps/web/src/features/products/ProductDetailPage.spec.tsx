...
describe('Product Detail V1', () => {
  ...
  it('renders a Product not-found state for an invalid slug', () => {
    ...
    expect(screen.getByRole('link', { name: 'Về danh sách sản phẩm' })).toHaveAttribute('href', '/products');
  });

  it('switches gallery media with accessible thumbnail controls', async () => {
    renderDetail();
    const ingredientMedia = screen.getByRole('button', { name: 'Xem Thông tin thành phần của Sữa yến mạch nguyên bản' });
    ...
    expect(screen.getByRole('img', { name: 'Thông tin thành phần của Sữa yến mạch nguyên bản' })).toBeInTheDocument();
  });

  it('renders nutrition, ingredients and dietary tags from typed data', () => {
    renderDetail();
    const nutrition = screen.getByRole('table', { name: 'Thông tin dinh dưỡng của Sữa yến mạch nguyên bản' });
    ...
    expect(screen.getByText('Có chứa yến mạch. Thông tin dị ứng chính thức cần đối chiếu trên nhãn sản phẩm.')).toBeInTheDocument();
  });
  ...
  it('renders related products using same-category presentation data', () => {
    ...
    expect(within(related as HTMLElement).getByText('Sữa hạnh nhân không đường')).toBeInTheDocument();
    expect(within(related as HTMLElement).getByRole('link', { name: 'Xem chi tiết Sữa hạt điều cacao' })).toHaveAttribute('href', '/products/cashew-cocoa-milk');
  });
  ...
  it('renders valid breadcrumb links for catalog and category', () => {
    ...
    expect(within(breadcrumb).getByRole('link', { name: 'Sản phẩm' })).toHaveAttribute('href', '/products');
    expect(within(breadcrumb).getByRole('link', { name: 'Sữa hạt' })).toHaveAttribute('href', '/products?category=plant-milk');
    expect(within(breadcrumb).getByText('Sữa yến mạch nguyên bản')).toHaveAttribute('aria-current', 'page');
  });

  it('renders loading and error foundations', async () => {
    const retry = vi.fn();
    const { rerender } = render(<MemoryRouter initialEntries={['/products/oat-milk-original']}><Routes><Route path="/products/:slug" element={<ProductDetailPage status="loading" />} /></Routes></MemoryRouter>);
    expect(screen.getByRole('status', { name: 'Đang tải chi tiết sản phẩm' })).toBeInTheDocument();
    rerender(<MemoryRouter initialEntries={['/products/oat-milk-original']}><Routes><Route path="/products/:slug" element={<ProductDetailPage status="error" onRetry={retry} />} /></Routes></MemoryRouter>);
