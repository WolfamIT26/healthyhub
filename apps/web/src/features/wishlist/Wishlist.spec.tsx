...
describe('Wishlist V1 frontend foundation', () => {
  ...
  it('allows an unverified Customer to use Wishlist', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Xóa Sữa yến mạch nguyên bản khỏi yêu thích' }),
    ).toBeInTheDocument();
  });
  ...
  it('renders the protected empty Wishlist page and discovery CTA', () => {
    ...
    renderFlow('/wishlist');
    expect(
      screen.getByRole('heading', { level: 1, name: 'Sản phẩm yêu thích' }),
    ).toBeInTheDocument();
  ...
  it('renders out-of-stock products without removing them from Wishlist', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Xóa Sữa chua dừa vị xoài khỏi yêu thích' }),
    ).toBeInTheDocument();
  });
