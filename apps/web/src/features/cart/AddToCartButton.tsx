  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        title={
          prompt === 'customer-only'
            ? 'Giỏ hàng chỉ dành cho tài khoản Customer.'
            : 'Đăng nhập để thêm sản phẩm vào giỏ hàng.'
        }
        description={
          prompt === 'customer-only'
            ? 'Tài khoản nội bộ không sử dụng Customer Cart flow.'
            : 'Cart được gắn với tài khoản Customer và không được lưu giả trên trình duyệt.'
        }
        confirmLabel={prompt === 'customer-only' ? 'Đóng' : 'Đăng nhập'}
      />
    </>
  );
}
