...
describe('EmailVerificationBanner', () => {
  beforeEach(() => {
    ...
    vi.mocked(useAuth).mockReturnValue({
      ...
      actor: { id: '1', email: 'pending@example.com', fullName: 'Pending Customer', roles: ['CUSTOMER'], isEmailVerified: false },
      current: null,
      login: vi.fn(), logout: vi.fn(), hasRole: vi.fn(), hasPermission: vi.fn(),
    });
  });
  ...
  it('shows for an unverified Customer, supports resend and can be dismissed temporarily', async () => {
    ...
    const { unmount } = render(<MemoryRouter><EmailVerificationBanner /></MemoryRouter>);
    expect(screen.getByText('⚠ Email của bạn chưa được xác minh.')).toBeInTheDocument();
    expect(screen.getByRole('link', { name: 'Xác minh ngay' })).toHaveAttribute('href', '/verify-email');
    ...
    unmount();
    render(<MemoryRouter><EmailVerificationBanner /></MemoryRouter>);
    expect(screen.getByText('⚠ Email của bạn chưa được xác minh.')).toBeInTheDocument();
  });
  ...
  it('does not show for a verified user', () => {
    vi.mocked(useAuth).mockReturnValue({
      status: 'authenticated',
      actor: { id: '1', email: 'verified@example.com', fullName: 'Verified', roles: ['CUSTOMER'], isEmailVerified: true },
      current: null,
      login: vi.fn(), logout: vi.fn(), hasRole: vi.fn(), hasPermission: vi.fn(),
    });
    render(<MemoryRouter><EmailVerificationBanner /></MemoryRouter>);
