...
export function AuthNavigation({ onNavigate }: { onNavigate?: () => void } = {}) {
  ...
  return <div className="flex flex-wrap items-center gap-2"><span className="hidden sm:inline">{auth.actor?.fullName}</span>{auth.hasRole('CUSTOMER') ? <><Link className="inline-flex min-h-11 items-center rounded-control px-3 font-semibold hover:bg-neutral-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" to="/wishlist" onClick={onNavigate}>Yêu thích</Link><Link className="inline-flex min-h-11 items-center rounded-control px-3 font-semibold hover:bg-neutral-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" to="/cart" onClick={onNavigate}>Giỏ hàng</Link></> : null}<button type="button" className="min-h-11 rounded-control px-3 font-semibold hover:bg-neutral-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" onClick={() => { void auth.logout().then(() => { onNavigate?.(); navigate('/login', { replace: true }); }); }}>Đăng xuất</button></div>;
}
