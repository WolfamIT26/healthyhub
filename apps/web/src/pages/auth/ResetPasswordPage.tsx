...
import { AuthCard, AuthForm, FormAlert, StateIllustration, SubmitButton } from '../../features/auth/AuthCard';
...
export function ResetPasswordPage() {
  ...
  return <AuthCard title="Đặt lại mật khẩu" description="Tạo mật khẩu mới cho tài khoản của bạn." footer={<Link to="/login">Quay lại đăng nhập</Link>}>
    {success ? <><StateIllustration src={authAssets.successIllustration} alt="Đổi mật khẩu thành công" /><FormAlert tone="success">Mật khẩu đã được thay đổi. Tất cả phiên cũ đã đăng xuất. Bạn có thể đăng nhập lại.</FormAlert></> : <AuthForm onSubmit={submit}>
      ...
      <PasswordField id="password" label="Mật khẩu mới" value={password} onChange={setPassword} error={errors.password} autoComplete="new-password" />
      <p className="field-hint">Dùng 12–128 ký tự, không chứa thông tin email dễ đoán và không sử dụng mật khẩu phổ biến.</p>
      <PasswordField id="confirmation" label="Xác nhận mật khẩu mới" value={confirmation} onChange={setConfirmation} error={errors.confirmation} autoComplete="new-password" />
