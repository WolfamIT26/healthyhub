...
export function ResetPasswordPage() {
  ...
  if (!token) return <AuthCard title="Liên kết không hợp lệ" description="Liên kết đặt lại mật khẩu thiếu token hoặc đã bị thay đổi."><FormAlert>Vui lòng yêu cầu một liên kết đặt lại mật khẩu mới.</FormAlert><div className="auth-footer"><Link to="/forgot-password">Yêu cầu liên kết mới</Link></div></AuthCard>;
  return <AuthCard title="Đặt lại mật khẩu" description="Tạo mật khẩu mới cho tài khoản của bạn." footer={<Link to="/login">Quay lại đăng nhập</Link>}>
    {success ? <FormAlert tone="success">Mật khẩu đã được thay đổi. Tất cả phiên cũ đã đăng xuất. Bạn có thể đăng nhập lại.</FormAlert> : <AuthForm onSubmit={submit}>
