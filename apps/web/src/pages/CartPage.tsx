  ...
  function updateQuantity(cartItemId: string, rawQuantity: number) {
    if (!Number.isInteger(rawQuantity) || rawQuantity < 1 || rawQuantity > 9999) {
      setQuantityErrors((current) => ({
        ...current,
        [cartItemId]: 'Số lượng phải là số nguyên từ 1 đến 9999.',
      }));
      return;
    }
  ...
  return (
    <main className="flex-1 bg-neutral-50">
      <div className="container py-8 sm:py-10">
        ...
        {cart.loading ? (
          <div className="mt-8 grid gap-4" aria-label="Đang tải giỏ hàng">
          ...
          <div className="mt-8">
            <ErrorState
              ...
              description="Kết nối tới Cart server không thành công."
              action={
                <Button
                  type="button"
                  onClick={() => {
          ...
          <div className="mt-8">
            <EmptyState
              ...
              description="Khám phá catalog để tìm sản phẩm phù hợp."
              action={
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px]">
            <section className="space-y-4" aria-label="Sản phẩm trong giỏ hàng">
              {items.map((item) => {
                const unavailable = !['AVAILABLE', 'LOW_STOCK'].includes(item.availability);
                const pending = cart.pendingItemId === item.id;
                return (
                  <Card key={item.id} className="grid gap-4 sm:grid-cols-[112px_minmax(0,1fr)]">
                    <div
                      className="flex aspect-square items-center justify-center rounded-card bg-primary-50 text-5xl"
                      role="img"
                      aria-label={`Minh họa ${item.name}`}
                    >
                      🛒
                    </div>
                    <div className="min-w-0">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <Link
                            ...
                            className="font-bold text-neutral-950 hover:text-primary-700"
                          >
                            {item.name}
                          </Link>
                        ...
                        <IconButton
                          ...
                          disabled={pending}
                          onClick={() => {
                            void cart.remove(item.id).catch(() => undefined);
                          }}
                        >
                          ×
                        </IconButton>
                      ...
                      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                        <div>
                          <p className="font-bold text-primary-700">
                            {moneyFormatter.format(Number(item.unitPrice))}
                          </p>
                          <p className="text-xs text-neutral-500">Đơn giá từ máy chủ</p>
                        </div>
                        <p
                          className={`text-sm font-semibold ${unavailable ? 'text-error-dark' : item.availability === 'LOW_STOCK' ? 'text-warning-dark' : 'text-success-dark'}`}
                        >
                      ...
                      <div className="mt-4 flex flex-wrap items-center gap-2">
                        ...
                        <input
                          ...
                          className="h-11 w-20 rounded-control border border-neutral-300 text-center"
                          type="number"
                          min="1"
                          max="9999"
                          step="1"
                      ...
                      {quantityErrors[item.id] ? (
                        <p className="mt-2 text-sm text-error-dark" role="alert">
                          {quantityErrors[item.id]}
                        </p>
                      ) : null}
                      {unavailable ? (
                        <Alert tone="error" className="mt-3">
                          Sản phẩm hiện không đủ điều kiện Checkout. Item không tự bị xóa.
                        </Alert>
                      ) : null}
                    </div>
                  </Card>
            ...
            <aside>
              <CartSummary
                ...
                subtotal={cart.cart?.subtotal ?? '0.00'}
                action={
                  hasUnavailableItem ? (
                    <Button type="button" className="w-full" disabled>
                      Checkout chưa khả dụng
                    </Button>
                  ) : auth.actor?.isEmailVerified === false ? (
                    <Button
                      type="button"
                      className="w-full"
                    ...
                    <Link to="/checkout" className={buttonClassName({ className: 'w-full' })}>
                      Tiến hành Checkout
                    </Link>
                  )
                }
      ...
      <Modal
        ...
        footer={
          <>
            ...
            >
              {resendState === 'pending' ? 'Đang gửi…' : 'Gửi lại email'}
            </Button>
            <Link to="/verify-email" className={buttonClassName()}>
              Xác minh ngay
            </Link>
          </>
        }
      >
        <p>Cart vẫn sử dụng được; chỉ bước Checkout yêu cầu email đã xác minh.</p>
        {resendState === 'accepted' ? (
          <p className="mt-2 text-success-dark" role="status">
            Email xác minh đã được gửi lại.
          </p>
        ) : null}
        {resendState === 'error' ? (
          <p className="mt-2 text-error-dark" role="alert">
            Không thể gửi lại email lúc này.
          </p>
        ) : null}
      </Modal>
    </main>
