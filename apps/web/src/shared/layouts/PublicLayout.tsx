    ...
    <div className="flex min-h-screen min-h-[100dvh] flex-col bg-slate-50 text-slate-950">
      <header className="relative z-50 shrink-0 border-b border-neutral-200 bg-white">
        <div className="container flex min-h-[72px] items-center justify-between gap-3 py-3">
          <NavLink to="/" className="inline-flex min-h-11 items-center gap-2 rounded-control font-bold text-primary-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" onClick={() => setMenuOpen(false)}>
            <img src={logoSymbol} alt="" className="h-10 w-10 object-contain" width="40" height="40" />
      ...
      <footer className="bg-neutral-950 py-12 text-neutral-300">
        <div className="container grid gap-8 sm:grid-cols-2 lg:grid-cols-5">
          <div className="sm:col-span-2 lg:col-span-1"><div className="flex items-center gap-2 text-lg font-bold text-white"><img src={logoSymbol} alt="" className="h-10 w-10 object-contain" loading="lazy" />{webEnv.appName}</div><p className="mt-3 text-sm leading-6">Nền tảng giúp bạn tìm hiểu và lựa chọn sản phẩm healthy thuận tiện hơn.</p></div>
          <FooterGroup title="Sản phẩm" items={[['Khám phá sản phẩm', '/products'], ['Danh mục', '/products'], ['Khuyến mãi', '/promotions']]} />
          <FooterGroup title="Hỗ trợ" items={[['AI hỗ trợ', '/ai'], ['Đăng nhập', '/login'], ['Đăng ký', '/register']]} />
          <FooterGroup title="Kiến thức" items={[['Blog HealthyHub', '/blog'], ['Trang chủ', '/']]} />
          <div><h2 className="font-bold text-white">Liên hệ & chính sách</h2><p className="mt-3 text-sm leading-6">Thông tin liên hệ và chính sách chính thức sẽ được hiển thị từ cấu hình storefront khi module tương ứng sẵn sàng.</p></div>
        </div>
        <div className="container mt-10 border-t border-neutral-800 pt-6 text-sm">© {new Date().getFullYear()} HealthyHub. Thông tin trên nền tảng không thay thế tư vấn chuyên môn.</div>
      </footer>
    </div>
  );
}
...
function FooterGroup({ title, items }: { title: string; items: Array<[string, string]> }) {
  return <div><h2 className="font-bold text-white">{title}</h2><ul className="mt-3 space-y-2 text-sm">{items.map(([label, to]) => <li key={`${label}-${to}`}><NavLink to={to} className="inline-flex min-h-11 items-center hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">{label}</NavLink></li>)}</ul></div>;
}
