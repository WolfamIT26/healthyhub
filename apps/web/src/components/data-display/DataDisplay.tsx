...
export function ProductCard({ name, category, price, originalPrice, imageUrl, imageFallback, badge, details, action }: { name: string; category?: string; price: string; originalPrice?: string; imageUrl?: string; imageFallback?: ReactNode; badge?: ReactNode; details?: ReactNode; action?: ReactNode }) { return <Card className="group flex h-full flex-col overflow-hidden p-0 transition-standard hover:-translate-y-1 hover:shadow-medium motion-reduce:hover:translate-y-0"><div className="aspect-square bg-gradient-to-br from-primary-50 via-white to-accent-light">{imageUrl ? <img className="h-full w-full object-cover" src={imageUrl} alt={name} loading="lazy" width="480" height="480" /> : <div className="flex h-full items-center justify-center text-7xl" role="img" aria-label={`Minh họa ${name}`}>{imageFallback ?? '🍃'}</div>}</div><div className="flex flex-1 flex-col p-4">{badge}<div className="flex-1">{category ? <p className="mt-3 line-clamp-1 text-xs font-semibold uppercase tracking-wide text-neutral-500">{category}</p> : null}<h3 className="mt-1 line-clamp-2 font-semibold text-neutral-950">{name}</h3><div className="mt-2 flex flex-wrap items-baseline gap-2"><p className="font-bold text-primary-700">{price}</p>{originalPrice ? <del className="text-sm text-neutral-500">{originalPrice}</del> : null}</div>{details ? <div className="mt-3 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-neutral-600">{details}</div> : null}</div>{action ? <div className="mt-4">{action}</div> : null}</div></Card>; }
