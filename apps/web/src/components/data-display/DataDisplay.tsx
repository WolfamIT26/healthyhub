...
export function ProductCard({ name, price, imageUrl, badge, action }: { name: string; price: string; imageUrl?: string; badge?: ReactNode; action?: ReactNode }) { return <Card className="overflow-hidden p-0"><div className="aspect-square bg-neutral-100">{imageUrl ? <img className="h-full w-full object-cover" src={imageUrl} alt={name} loading="lazy" /> : null}</div><div className="p-4">{badge}<h3 className="mt-2 line-clamp-2 font-semibold text-neutral-950">{name}</h3><p className="mt-2 font-bold text-primary-700">{price}</p>{action ? <div className="mt-4">{action}</div> : null}</div></Card>; }
