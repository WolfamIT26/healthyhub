...
export function Label({
  ...
  return (
    <label htmlFor={htmlFor} className={cn('text-sm font-semibold text-neutral-800', className)}>
      {children}
      {required ? (
...
export function FieldError({ id, children }: { id?: string; children?: ReactNode }) {
  ...
  return (
    <p id={id} className="text-sm text-error-dark">
      {children}
    </p>
  );
}
  ...
  return (
    <div className={cn('flex flex-col gap-1.5', className)}>
      ...
      {error ? (
        ...
        <p id={`${id}-success`} className="text-sm text-success-dark" role="status">
          {success}
        </p>
      ) : helperText ? (
        <p id={`${id}-helper`} className="text-xs leading-5 text-neutral-500">
          {helperText}
        </p>
      ) : null}
    </div>
...
function Choice({
  ...
  return (
    <label
      className={cn(
        'inline-flex min-h-11 cursor-pointer items-center gap-2 text-sm text-neutral-800',
        className,
      )}
    >
      <input
...
export function Switch({
  ...
  return (
    <div className="inline-flex min-h-11 items-center gap-3 text-sm text-neutral-800">
      <button
        id={id}
        type="button"
        role="switch"
        ...
        className={cn(
          'relative h-6 w-11 rounded-full transition-standard focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:opacity-60',
          checked ? 'bg-primary' : 'bg-neutral-300',
        )}
      >
        <span
          className={cn(
            'absolute top-1 h-4 w-4 rounded-full bg-white shadow-sm transition-standard',
  ...
  function SearchInput({ className, onClear, value, ...props }, ref) {
    return (
      <div className="relative">
        ...
        />
        <svg
          className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 fill-none stroke-neutral-500"
          viewBox="0 0 24 24"
          aria-hidden="true"
        >
