...
const activeProduct = {
  id: '101', productName: 'Sữa yến mạch', slug: 'sua-yen-mach', basePrice: '69000.00',
  productStatus: 'active', productVisibility: 'public', sellableStatus: 'sellable', deletedAt: null,
};
...
describe('Cart minimum executable dependencies', () => {
  it('reads authoritative Product identity, price and sellable state from repository', async () => {
    ...
    await expect(reader.getProductCommerceSnapshot('101')).resolves.toEqual({
      productId: '101', name: 'Sữa yến mạch', slug: 'sua-yen-mach', currentPrice: '69000.00', currency: 'VND', sellable: true,
    });
  ...
  it('returns missing and non-sellable Product without any fallback authority', async () => {
    const repository = { findById: vi.fn().mockResolvedValueOnce(null).mockResolvedValueOnce({ ...activeProduct, productVisibility: 'hidden' }) };
    const reader = new ProductCommerceReader(repository as never);
  ...
  ])('evaluates Inventory availability without mutation or fake fallback', async (item, quantity, status) => {
    const reader = new InventoryAvailabilityReader({ findByProductId: vi.fn().mockResolvedValue(item) } as never);
  ...
  it('rejects invalid quantity and treats a missing Inventory row as unavailable', async () => {
    const reader = new InventoryAvailabilityReader({ findByProductId: vi.fn().mockResolvedValue(null) } as never);
    await expect(reader.checkAvailability('101', 0)).resolves.toEqual({ status: 'INVALID_QUANTITY', availableQuantity: null });
    await expect(reader.checkAvailability('101', 1)).resolves.toEqual({ status: 'UNAVAILABLE', availableQuantity: null });
  });
  ...
  it('resolves verified or unverified authenticated Customers only from actor identity', async () => {
    ...
    await expect(resolver.resolve({ userAccountId: '42', roles: ['CUSTOMER'] } as never)).resolves.toEqual({ customerProfileId: '501', userAccountId: '42' });
  ...
  it('rejects internal actors and missing CustomerProfile mappings', async () => {
    ...
    const resolver = new CustomerOwnerResolver(repository as never);
    await expect(resolver.resolve({ userAccountId: '7', roles: ['ADMINISTRATOR'] } as never)).rejects.toMatchObject({ code: 'CUSTOMER_ROLE_REQUIRED' });
    await expect(resolver.resolve({ userAccountId: '42', roles: ['CUSTOMER'] } as never)).rejects.toBeInstanceOf(CustomerOwnerResolutionError);
  });
});
