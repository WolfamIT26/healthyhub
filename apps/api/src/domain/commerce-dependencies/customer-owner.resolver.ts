...
export interface CustomerOwnerRepository {
  findActiveByUserAccountId(userAccountId: string): Promise<CustomerProfileEntity | null>;
}
...
export class CustomerOwnerResolutionError extends Error {
  constructor(public readonly code: 'CUSTOMER_ROLE_REQUIRED' | 'CUSTOMER_PROFILE_NOT_FOUND') {
    super(code);
  }
}
...
export class CustomerOwnerResolver {
  constructor(
    @Inject(CUSTOMER_OWNER_REPOSITORY) private readonly repository: CustomerOwnerRepository,
  ) {}

  async resolve(
    auth: AuthenticatedRequestContext,
  ): Promise<{ customerProfileId: string; userAccountId: string }> {
    if (!auth.roles.includes('CUSTOMER'))
      throw new CustomerOwnerResolutionError('CUSTOMER_ROLE_REQUIRED');
