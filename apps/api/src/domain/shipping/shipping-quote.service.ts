...
const MANUAL_METHOD = Object.freeze({ code: 'manual' as const, name: 'Giao hàng tiêu chuẩn HealthyHub' });
...
export class ShippingQuoteService {
  ...
  quote(address: ShippingAddressInput, methodCode: string, cart: ShippingCartContext): ShippingQuote {
    this.assertCart(cart);
    if (methodCode !== MANUAL_METHOD.code) throw new UnsupportedShippingMethodError('Phương thức giao hàng chưa được hỗ trợ.');
    const addressSnapshot = this.createAddressSnapshot(address);
    if (!this.isServiceable(addressSnapshot)) throw new ShippingUnavailableError('Địa chỉ nằm ngoài phạm vi Shipping V1.');
    return {
  ...
  validateQuote(reference: string, address: ShippingAddressInput, methodCode: string, cart: ShippingCartContext): boolean {
  ...
  createAddressSnapshot(input: ShippingAddressInput): ShippingAddressSnapshot {
    ...
    if (
      recipientName.length < 1 || recipientName.length > 255 || !PHONE_PATTERN.test(phone) ||
      countryCode.length !== 2 || provinceCity.length < 1 || provinceCity.length > 150 ||
      district.length < 1 || district.length > 150 || addressLine.length < 1 || addressLine.length > 500 ||
      (ward?.length ?? 0) > 150 || (note?.length ?? 0) > 500
    ) throw new ShippingValidationError('Địa chỉ giao hàng không hợp lệ.');
    return Object.freeze({
      recipientName, phone, countryCode, provinceCity, district,
      ward, addressLine, note,
    });
  }
  ...
  private assertCart(cart: ShippingCartContext): void {
    if (!cart.cartId || !cart.isValid || !Number.isSafeInteger(cart.itemCount) || cart.itemCount < 1 ||
      !MONEY_PATTERN.test(cart.subtotal) || Number(cart.subtotal) < 0) {
