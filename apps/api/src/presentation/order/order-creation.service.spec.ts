...
const address = { recipientName: 'Nguyễn Văn A', phone: '0901234567', countryCode: 'VN',
  provinceCity: 'Hồ Chí Minh', district: 'Quận 1', ward: 'Bến Nghé', addressLine: '12 Nguyễn Huệ', note: 'Gọi trước' };
const cart = { id: '10', customerProfileId: '7', cartStatus: 'active' };
const persistedItem = { id: 'cart-item-1', productId: '1', quantity: 2 };
const product = { productId: '1', productCode: 'HH-0001', name: 'Sữa yến mạch', slug: 'sua-yen-mach',
  currentPrice: '69000.00', currency: 'VND', sellable: true };

function setup() {
  const shipping = new ShippingQuoteService();
  const repository = { findByIdempotency: vi.fn().mockResolvedValue(null), createSnapshot: vi.fn() };
  const carts = { findActive: vi.fn().mockResolvedValue(cart), listActiveItems: vi.fn().mockResolvedValue([persistedItem]) };
  const authentication = { findAccountById: vi.fn().mockResolvedValue({ emailVerifiedAt: new Date(), userStatus: 'active' }) };
  ...
  const quote = shipping.quote(address, 'manual', { cartId: '10', subtotal: '138000.00', itemCount: 2, isValid: true });
  const input = { shippingAddress: address, shippingMethod: 'manual' as const,
    shippingQuoteReference: quote.quoteReference, paymentMethod: 'cod' as const };
...
describe('OrderCreationService', () => {
  it('creates a verified Customer Order from authoritative Cart/Product/Shipping/COD snapshots', async () => {
    ...
    await expect(service.createOrderFromCheckout(auth, 'checkout-attempt-001', input)).resolves.toMatchObject({
      status: 'new', paymentStatus: 'pending', paymentMethod: 'cod', shippingMethod: 'manual',
      subtotal: '138000.00', shippingFee: '0.00', total: '138000.00', currency: 'VND',
      items: [{ productId: '1', productName: 'Sữa yến mạch', sku: 'HH-0001', unitPrice: '69000.00', quantity: 2, lineTotal: '138000.00' }],
    });
  ...
  it('rejects unverified Customer before creating an Order', async () => {
    ...
    authentication.findAccountById.mockResolvedValue({ emailVerifiedAt: null, userStatus: 'pending' });
    await expect(service.createOrderFromCheckout(auth, 'checkout-attempt-001', input)).rejects.toMatchObject({ response: { code: 'AUTH.EMAIL_NOT_VERIFIED' } });
  ...
  it('rejects an internal actor and never accepts owner identity from input', async () => {
    ...
    await expect(service.createOrderFromCheckout(auth, 'checkout-attempt-001', { ...input, customerId: '999' } as never))
      .rejects.toMatchObject({ response: { code: 'ORDER.ACCESS_DENIED' } });
    expect(repository.createSnapshot).not.toHaveBeenCalled();
  });

  it.each([
    ['missing Cart', (state: ReturnType<typeof setup>) => state.carts.findActive.mockResolvedValue(null), 'ORDER.CART_EMPTY'],
    ['empty Cart', (state: ReturnType<typeof setup>) => state.carts.listActiveItems.mockResolvedValue([]), 'ORDER.CART_EMPTY'],
    ['unavailable Product', (state: ReturnType<typeof setup>) => state.products.findSellableProduct.mockResolvedValue(null), 'ORDER.CART_INVALID'],
    ['insufficient stock', (state: ReturnType<typeof setup>) => state.inventory.checkAvailability.mockResolvedValue({ status: 'INSUFFICIENT_STOCK', availableQuantity: 1 }), 'ORDER.INSUFFICIENT_STOCK'],
  ])('rejects %s', async (_label, mutate, code) => {
    const state = setup(); mutate(state);
    await expect(state.service.createOrderFromCheckout(auth, 'checkout-attempt-001', state.input)).rejects.toMatchObject({ response: { code } });
  });

  it('rejects stale Shipping quote and unsupported Shipping/Payment methods', async () => {
    const stale = setup();
    await expect(stale.service.createOrderFromCheckout(auth, 'checkout-attempt-001',
      { ...stale.input, shippingQuoteReference: 'shq_stale' })).rejects.toMatchObject({ response: { code: 'ORDER.SHIPPING_INVALID' } });
    const shippingMethod = setup();
    await expect(shippingMethod.service.createOrderFromCheckout(auth, 'checkout-attempt-002',
      { ...shippingMethod.input, shippingMethod: 'express' } as never)).rejects.toMatchObject({ response: { code: 'ORDER.SHIPPING_INVALID' } });
    const paymentMethod = setup();
    await expect(paymentMethod.service.createOrderFromCheckout(auth, 'checkout-attempt-003',
      { ...paymentMethod.input, paymentMethod: 'online' } as never)).rejects.toMatchObject({ response: { code: 'ORDER.PAYMENT_METHOD_UNSUPPORTED' } });
  });
  ...
  it('returns the same Order for the same idempotency key and identical request', async () => {
    ...
    state.repository.findByIdempotency.mockResolvedValue(aggregate({ requestHash: createdInput.requestHash }));
    await expect(state.service.createOrderFromCheckout(auth, 'checkout-attempt-001', state.input))
  ...
  it('rejects conflicting payload reuse of an idempotency key', async () => {
    const state = setup();
    state.repository.findByIdempotency.mockResolvedValue(aggregate({ requestHash: 'different-request' }));
    await expect(state.service.createOrderFromCheckout(auth, 'checkout-attempt-001', state.input))
  ...
  it('keeps persisted Order snapshots unchanged when Product authority later changes', async () => {
    const state = setup();
    const created = await state.service.createOrderFromCheckout(auth, 'checkout-attempt-001', state.input);
