...
export class TypeOrmProductCommerceRepository implements ProductCommerceRepository {
  ...
  constructor(dataSource: DataSource) { this.products = dataSource.getRepository(ProductEntity); }
  findById(productId: string) { return this.products.findOne({ where: { id: productId } }); }
}
...
export class TypeOrmInventoryAvailabilityRepository implements InventoryAvailabilityRepository {
  ...
  constructor(dataSource: DataSource) { this.inventory = dataSource.getRepository(InventoryItemEntity); }
  findByProductId(productId: string) { return this.inventory.findOne({ where: { productId } }); }
}
...
export class TypeOrmCustomerOwnerRepository implements CustomerOwnerRepository {
  ...
  constructor(dataSource: DataSource) { this.customers = dataSource.getRepository(CustomerProfileEntity); }
  findActiveByUserAccountId(userAccountId: string) { return this.customers.findOne({ where: { userAccountId, customerStatus: 'active' } }); }
}
