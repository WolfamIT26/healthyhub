...
@Index('uq_payment_attempts_provider_reference', ['tenantId', 'provider', 'providerReference'], {
  unique: true,
})
@Index('uq_payment_attempts_idempotency', ['tenantId', 'paymentId', 'idempotencyKeyHash'], {
  unique: true,
})
...
export class PaymentAttemptEntity extends BaseAuditEntity {
  ...
  @Column({ name: 'provider_transaction_no', type: 'varchar', length: 32, nullable: true })
  ...
  @Column({ name: 'attempt_status', type: 'varchar', length: 32, default: 'pending' })
  attemptStatus!: 'pending' | 'paid' | 'failed' | 'cancelled';
