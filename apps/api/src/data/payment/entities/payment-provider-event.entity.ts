...
@Index('uq_payment_provider_events_identity', ['tenantId', 'provider', 'providerEventId'], { unique: true })
...
export class PaymentProviderEventEntity extends BaseAuditEntity {
  ...
  @Column({ name: 'payment_id', type: 'bigint', unsigned: true, nullable: true }) paymentId!: string | null;
  ...
  @Column({ name: 'processing_status', type: 'varchar', length: 32, default: 'received' }) processingStatus!: WebhookProcessingStatus;
  @Column({ name: 'received_at', type: 'datetime', precision: 3 }) receivedAt!: Date;
  @Column({ name: 'processing_started_at', type: 'datetime', precision: 3, nullable: true }) processingStartedAt!: Date | null;
  @Column({ name: 'processed_at', type: 'datetime', precision: 3, nullable: true }) processedAt!: Date | null;
