...
export class CustomerProfileEntity extends BaseAuditEntity {
  ...
  @Column({ name: 'user_account_id', type: 'bigint', unsigned: true, nullable: true }) userAccountId!: string | null;
  ...
  @Column({ name: 'contact_info', type: 'json', nullable: true }) contactInfo!: { email?: string; phone?: string } | null;
  @Column({ name: 'customer_status', type: 'varchar', length: 32, default: 'active' }) customerStatus!: 'active' | 'guest' | 'blocked' | 'archived';
  @Column({ name: 'consent_state', type: 'varchar', length: 32, default: 'unknown' }) consentState!: 'unknown' | 'granted' | 'denied';
  @Column({ name: 'marketing_opt_in_status', type: 'varchar', length: 32, default: 'not_opted_in' }) marketingOptInStatus!: 'not_opted_in' | 'opted_in' | 'opted_out';
}
