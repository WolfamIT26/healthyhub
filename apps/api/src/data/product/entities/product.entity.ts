...
export class ProductEntity extends BaseAuditEntity {
  ...
  @Column({ name: 'brand_id', type: 'bigint', unsigned: true, nullable: true }) brandId!: string | null;
  ...
  @Column({ name: 'base_price', type: 'decimal', precision: 12, scale: 2, default: '0.00' }) basePrice!: string;
  @Column({ name: 'sellable_status', type: 'varchar', length: 32, default: 'unavailable' }) sellableStatus!: 'sellable' | 'out_of_stock' | 'preorder' | 'unavailable';
  @Column({ name: 'product_visibility', type: 'varchar', length: 32, default: 'hidden' }) productVisibility!: 'public' | 'hidden' | 'private';
  @Column({ name: 'product_status', type: 'varchar', length: 32, default: 'draft' }) productStatus!: 'draft' | 'active' | 'discontinued';
}
