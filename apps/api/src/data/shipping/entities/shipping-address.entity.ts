...
@Index(
  'uq_shipping_addresses_tenant_shipment_status',
  ['tenantId', 'shipmentId', 'addressSnapshotStatus'],
  { unique: true },
)
export class ShippingAddressEntity extends BaseAuditEntity {
  ...
  @Column({ name: 'customer_address_id', type: 'bigint', unsigned: true, nullable: true })
  ...
  @Column({ name: 'delivery_note', type: 'varchar', length: 500, nullable: true }) deliveryNote!:
    string | null;
  @Column({ name: 'address_snapshot_status', type: 'varchar', length: 32, default: 'active' })
