  ...
  it('deduplicates concurrent delivery, commits once, supports failed retry, rejection and reload', async () => {
    ...
    const user = await userRepo.save(
      userRepo.create({
        ...
        normalizedEmail: `payment-${suffix}@example.test`,
        phone: null,
        displayName: 'Payment Fixture',
        passwordHash: 'integration-fixture-not-login-secret',
        userStatus: 'active',
    ...
    try {
      ...
      await repository.completeWithBusinessEffect(claimed.event.id, payment.id, async (manager) => {
        ...
        await manager
          ...
          .update(payment.id, { providerReference: input.providerReference });
      });
      expect(effects).toBe(1);
      expect((await repository.claim(input)).claimed).toBe(false);
      expect(
        (
          await new TypeOrmPaymentProviderEventRepository(dataSource).findByProviderEvent(
            'vnpay',
      ...
      const retryInput = {
        ...input,
        providerEventId: `retry-${suffix}`,
        payloadHash: 'd'.repeat(64),
      };
      const retryFirst = await repository.claim(retryInput);
      await expect(
        repository.completeWithBusinessEffect(retryFirst.event.id, payment.id, async (manager) => {
          await manager
            .getRepository(PaymentEntity)
            .update(payment.id, { providerReference: 'must-roll-back' });
          throw new Error('simulated business failure');
        }),
      ).rejects.toThrow('simulated business failure');
      expect((await paymentRepo.findOneByOrFail({ id: payment.id })).providerReference).toBe(
